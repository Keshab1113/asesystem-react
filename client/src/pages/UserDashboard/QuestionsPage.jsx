...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    // -------------------------------------------------------------------------
    // ANDROID-SPECIFIC HANDLING (COMPLETE VERSION)
    // -------------------------------------------------------------------------
    if (Device.isAndroid) {
      let backPressCount = 0;
      let backPressTimer = null;
      ...
      const handlePageShow = (e) => {
        if (e.persisted) {
          ...
          // Re-enter fullscreen when page is restored from cache
          const el = document.documentElement;
          if (el.requestFullscreen) {
            el.requestFullscreen().catch((err) =>
      ...
      // Fullscreen exit detection - ONLY trigger if NOT caused by back button
      const exitHandler = () => {
        if (
          ...
          !isSubmitting &&
          backPressCount === 0 // Don't terminate if back button was pressed
        ) {
          handleSubmit(
            true,
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const response = await api.post("/api/quiz-assignments/end", {
        ...
        status: status,
        user_ended_at: userLocalEndTime, // send local time
